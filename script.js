...
const staticTranslations = {
    en: { nav_home: 'Home', nav_home_alt: '← Back to Shop', nav_new: 'New Arrivals', hero_title: 'Discover Our Collection', hero_subtitle: 'Timeless designs.', sizes_title: 'Available Sizes', order_btn: 'Order via WhatsApp', order_btn_disabled: 'Select a size', size_placeholder: 'Select a Size...', footer_text: '© 2025 Kati Shop.' },
    fr: { nav_home: 'Accueil', nav_home_alt: '← Retour', nav_new: 'Nouveautés', hero_title: 'Découvrez Notre Collection', hero_subtitle: 'Des designs intemporels.', sizes_title: 'Tailles Disponibles', order_btn: 'Commander via WhatsApp', order_btn_disabled: 'Choisissez une taille', size_placeholder: 'Choisissez une taille...', footer_text: '© 2025 Kati Shop.' },
    ar: { nav_home: 'الرئيسية', nav_home_alt: '→ رجوع', nav_new: 'وصل حديثاً', hero_title: 'اكتشف مجموعتنا', hero_subtitle: 'تصاميم خالدة.', sizes_title: 'المقاسات المتوفرة', order_btn: 'اطلب عبر واتساب', order_btn_disabled: 'اختر مقاسًا أولاً', size_placeholder: 'اختر مقاس...', footer_text: '© 2025 كاتي شوب.' }
};
const waMessages = {
...
function runIndexPageLogic() {
    ...
    function translateIndexPage(lang) {
        ...
        // ** CHANGE: This no longer touches document.title **
...
async function runProductPageLogic() {
    ...
    if (!doc.exists) { container.innerHTML = '<h1>Product not found._</h1>'; return; }
    const product = doc.data();
    ...
    const sizesHTML = !hasSizes ? '' : `
        <div class="size-selector-container"><h3>${staticTranslations[currentLanguage].sizes_title}</h3><select id="size-selector"><option value="" disabled selected>${staticTranslations[currentLanguage].size_placeholder}</option>${product.sizes.split(',').map(s => s.trim()).map(size => `<option value="${size}">${size}</option>`).join('')}</select></div>`;
    ...
    container.innerHTML = `<div class="product-layout"><div class="product-gallery"><div class="main-image-container"><img src="${product.img_main}" id="main-product-image"></div><div class="thumbnail-container">${thumbnailsHTML}</div></div><div class="product-details"><h1 class="product-title">${name}</h1><p class="product-details-price">${price}</p><p class="product-description">${desc.replace(/\n/g, '<br>')}</p>${sizesHTML}<a href="#" id="order-button" class="add-to-cart-btn disabled" target="_blank">${staticTranslations[currentLanguage].order_btn_disabled}</a></div></div>`;
    ...
    if (!hasSizes) {
        ...
        sizeSelector.addEventListener('change', (e) => {
            if (e.target.value) updateWhatsAppLink(e.target.value);
        });
    }
